...
全画面でcontext menu、F12、主要browserのDeveloper Toolsとsource表示shortcutを抑止する。`outerWidth/outerHeight`と`innerWidth/innerHeight`の差からdockされたDeveloper Toolsを750msごとに推定し、検出中は操作画面を覆う。試験開始時にも検査し、閾値を超える時はsessionを作らない。browser menu、undockされたDeveloper Tools、改造browser、確実には阻止・検出できない。推定を環境完全性の証明に使わない。
...
提出確定後、Keyboard Lockを解除し、意図的終了としてFullscreenを抜ける。この離脱、不審eventへ加えない。最終画面から再挑戦、または開始画面へ戻れる。再挑戦、新しいsession、token、challenge、回答、証跡chainを作る。前回session、確認画面に残す。
...
PWA manifest、app identity、standalone表示、theme、192/512 iconを定義する。Service Worker、install判定のために登録し、全requestをnetworkへそのまま渡す。cacheとoffline fallback、持たない。server authorityが必要な試験をofflineへ静かに劣化させない。
